...
# load the input image, then grab the dimensions of the input image and crop the image such that it tiles nicely
...
# resize the input image using bicubic interpolation then write the baseline image to disk
...
for y in range(0, h - config.INPUT_DIM + 1, config.LABEL_SIZE):
	for x in range(0, w - config.INPUT_DIM + 1, config.LABEL_SIZE):
		...
		# make a prediction on the crop and store it in our output image
